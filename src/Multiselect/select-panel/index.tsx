...
const SelectPanel = () => {
  ...
  const [allOptins,setallOptions] = useState(options);
  ...
  const selectAllOptions = (checked) => {
  ...
  const [isAllSelected, hasAllSelected] = useMemo(() => {
    console.log(filteredOptions);
    const filteredOptionsList = allOptins.filter((o) => !o.disabled);
    return [
      filteredOptionsList.every(
          (o) => value.findIndex((v) => v.value === o.value) !== -1
      ),
  ...
  return (
    <div className="select-panel" role="listbox" ref={listRef}>
      ...
      {hasSelectAll && hasSelectableOptions && searchText && (
          <SelectItem
              tabIndex={skipIndex === 1 ? 0 : 1}
              checked={isAllOptionSelected}
              option={ {
                label: isAllOptionSelected ? 'UnSelect all filtered results' :  t("selectAllFiltered") ,
                value: "",
              }}
